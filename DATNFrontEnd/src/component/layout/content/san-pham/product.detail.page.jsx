...
import { fetchDataColor, fetchDataSize, findByProductId } from '../../../../service/api.service';
                    ...
                    <div className="product-details">
                        ...
                        <div className="select-size">
                            ...
                            <div className="size-options">
                                {size.filter(size => size.status === 1).map(size => (
                                    <button
                                        key={size.id}
                                        className={`size-button ${selectedSize === size.code ? 'selected' : ''} ${!availableSizes.includes(size.code) ? 'disabled-size' : ''}`}
                        ...
                        <div className="select-color">
                            ...
                            <div className="color-options">
                                {color.filter(color => color.status === 1).map(color => (
                                    <button
                                        key={color.id}
                                        className={`color-button ${selectedColor === color.name ? 'selected' : ''} ${!availableColors.includes(color.name) ? 'disabled-color' : ''}`}
