...
const OrderTable = (props) => {
        ...
        if (nextStatus === "shipping") {
          ...
          const productResponses = await Promise.all(
            productIds.map(async (id) => {
              try {
                ...
                console.error(
                  `Không thể lấy thông tin sản phẩm với ID: ${id}`,
                  error
                );
          ...
          const productsData = orderDetailResponses.map((detail) => {
            const productResponse = productResponses.find(
              (product) =>
                product?.data?.data?.id === detail.productDetailId.productId
            );
            ...
            return {
              product: {
                ...
                categoryName:
                  productResponse?.data?.data?.categoryName || "Unknown",
              },
          ...
          if (createOrderGhnResponse?.error) {
            throw new Error(
              createOrderGhnResponse?.error || "Giao hàng không thành công."
            );
          }
        }
      } else {
  ...
  const columnsHistoryPayment = [
    ...
    },
    {
      ...
      key: "status",
      render: (text, record) => {
        return record.status === 1 ? "Đã thanh toán" : "Chưa thanh toán";
      },
    },
  ];
