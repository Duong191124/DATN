    ...
    <MainContainer>
      <VoucherHeader>
        <HeaderTitle>
          ...
          <VoucherCount>
            {getActiveVoucherCount()} {t("MES-189")}
          </VoucherCount>
        </HeaderTitle>
      </VoucherHeader>
      <VoucherContainer>
            ...
            voucher.status === 1 && ( // Check if status is 1
              <VoucherCard>
                ...
                >
                      ...
                      <div>
                        ...
                        <DiscountTag color="#1a1a1a">
                          {voucher.discountPercent > 0
                            ? `${t("MES-190")} ${voucher.discountPercent}%`
                            : `${t("MES-190")} ${formatMinSpend(
                                voucher.discountAmount
                              )}`}
                          {voucher.maxDiscount &&
                            ` (${t("MES-191")} ${formatMinSpend(
                              voucher.maxDiscount
                            )})`}
                        </DiscountTag>
                      </div>
                  ...
                  <Text type="secondary">
                    {t("MES-193")}: {formatMinSpend(voucher.minPurchaseAmount)}{" "}
                    - {t("MES-194")}:{" "}
                    {voucher.maxDiscountAmount
                      ? formatMinSpend(voucher.maxDiscountAmount)
                      : ""}
                  </Text>
                </Space>
              </VoucherCard>
      ...
      <Modal
        title={<Title level={4}>{t("MES-195")}</Title>}
        open={modalVisible}
      ...
      >
        {selectedVoucher && (
          <Space direction="vertical" size="middle" style={{ width: "100%" }}>
            ...
            <div>
              <Text strong>{t("MES-196")}:</Text>
              <div>
                {selectedVoucher.discountPercent > 0 && (
                  <DiscountTag color="#1a1a1a">
                    {t("MES-190")} {selectedVoucher.discountPercent}%
                ...
                {selectedVoucher.discountAmount > 0 && (
                  <DiscountTag color="#1a1a1a">
                    {t("MES-190")}{" "}
                    {formatMinSpend(selectedVoucher.discountAmount)}
                    {selectedVoucher.maxDiscount &&
                      ` (${t("MES-191")} ${formatMinSpend(
                        selectedVoucher.maxDiscount
                      )})`}
            ...
            <div>
              <Text strong>{t("MES-197")}:</Text>
              <ul style={{ marginTop: "8px", paddingLeft: "20px" }}>
                <li>
                  {t("MES-193")}:{" "}
                  {formatMinSpend(selectedVoucher.minPurchaseAmount)}
                </li>
            ...
            <div>
              <Text strong>{t("MES-198")}:</Text>
              <p style={{ marginTop: "4px" }}>
                {t("MES-199")}{" "}
                {moment(selectedVoucher.expiryDate).format("DD/MM/YYYY")}
              </p>
            </div>
          </Space>
        )}
